...
$(etc)/max/vis$(1): $(rast)/vis$(1)_9
	...
	max=$$$$(for p in `cg.previous.mapsets --days=14 rast=vis$(1)_9 --delim=' '`; do \
	  r.info -r $$$$p | sed -n -e 's/max=// p'; \
	done | sort -n | tail -1); \
	echo $$$$max > $$@

$(rast)/p$(1): $(rast)/vis$(1)
	maps=`cg.previous.mapsets --quote --delim=',' rast=vis$(1)`; \
...
########################################################################
# Linke Turbidity
# This define could be called in the shell....
# tl=$(call tl,${MAPSET});, but we currently don't do that.
define tl
$$(ymd=`date --date="$1" --rfc-3339=date`; day=`date --date=$$ymd +%d`; while [[ $$day != "07" ]] && [[ $$day != "15" ]] && [[ $$day != "21" ]] ; do ymd=`date --date="$$ymd - 1 day" --rfc-3339=date`; day=`date --date=$$ymd +%d`; done ; date --date=$$ymd +tl_%m-%d@heliosat)
endef

# Instead we calculate the tl directly
tl_mapset:=$(shell ymd=`date --date=${MAPSET} --rfc-3339=date`; day=`date --date=$$ymd +%d`; while [[ $$day != "07" ]] && [[ $$day != "15" ]] && [[ $$day != "21" ]] ; do ymd=`date --date="$$ymd - 1 day" --rfc-3339=date`; day=`date --date=$$ymd +%d`; done ; date --date=$$ymd +tl_%m-%d@heliosat)


${rast}/Bk ${rast}/Dk ${rast}/K ${rast}/G: ${rast}/ssha $(patsubst %,${rast}/%,$(kXXXX)) ${rast}/Bc ${rast}/Dc ${rast}/Gc
	cg.daily.k ssha=ssha k="$(kXXXX)" tl=${tl_mapset} Bc=Bc Dc=Dc Gc=Gc
...
${rast}/Bc ${rast}/Dc ${rast}/Gc ${rast}/Trb ${rast}/Trd: ${rast}/ssha
	...
	r.heliosat --prefix='_' --noinstant --date=$(MAPSET) elevin=Z@2km linkein=${tl_mapset} lat=latitude_deg@2km ssha=ssha Bc=Bc Dc=Dc Gc=Gc Trb=Trb Trd=Trd
